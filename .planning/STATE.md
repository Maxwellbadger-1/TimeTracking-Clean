...
- **Next action:** **Phase 14.2 ist abgeschlossen.** Alle 13 Pläne sind gefahren, alle elf Befunde (F-1 bis F-8, D-1, D-2, V-1) plus B-1 geschlossen — je Befund ein unvermischter, einzeln revertierbarer Commit-Satz. Alle vier Gates grün (Server-`tsc` 0, Desktop-`tsc` 0, `check:rules` 0 mit fünf Gruppen und 69 Tests, `vitest run` **578 grün / 3 rot** — genau die drei vorbestehenden). E2E **21 grün / 0 rot / 2 übersprungen**, Exitcode 0. D-01 über die ganze Phase: fünf von fünf geschützten Tabellen mit identischer Zeilenzahl **und** SHA-256. Nächster Schritt ist die Verifikation der Phase. **Von den zehn ROADMAP-Erfolgskriterien sind acht erfüllt und zwei ausdrücklich eingeschränkt erfüllt:** das E2E-Kriterium (die Zahl stimmt, aber nur nach dem Freiräumen von 19 Altlast-Testnutzern — ohne diesen Schritt liefert derselbe Lauf 12/9/2; zwei Fälle tragen weiterhin `test.skip`) und „ein Vorgang, ein Name" (Schaltfläche und Meldung sagen „storniert", die Datenbank speichert weiterhin `rejected` — Weg A nach D-09). **45 neue Punkte warten auf eine Entscheidung des Anwenders: 14.2-U1 bis 14.2-U45** in `14-UAT-SAMMLUNG.md`, Abschnitt „Phase 14.2" (12 Entscheidungen mit Datenfolge, 13 Urteile am Bildschirm, 13 technische Befunde, 7 Kenntnisnahmen) — darunter Weg B zu F-7 (14.2-U1), der Tastaturverlust aus V-1 (14.2-U2), die E2E-Spec-Dateien ohne Aufräumschritt (14.2-U26) und die fünf Abnahmebefunde NB-A bis NB-E (14.2-U5 bis 14.2-U9). Unverändert offen daneben: die 36 Punkte der Phase 14.1 und die 86 der Phasen 11 bis 13.
...
- **Last completed:** Plan 14.2-10 — **D-1 geschlossen.** Alle sechs Kontrastunterschreitungen erreichen **gemessen** mindestens 4,5:1 in ihrem Modus: Saldoänderung im Vorschaupanel **3,18 → 4,84** (hell), positive Stundenwerte im Kontoauszug **3,30 → 4,84 bis 5,02** (hell, alle neun Stellen **einzeln** gemessen), Pflichtfeld-Stern **3,60–3,76 → 4,62/4,83** (hell) und **3,90 → 5,31** (dunkel), Primärknopf **3,68 → 5,17** (dunkel), Gefahrenknopf **3,76 → 4,83** (dunkel). **Gemessen heißt gemessen:** `getComputedStyle`, Elternkette bis zur ersten deckenden Ebene, Alphawerte per Source-over **verrechnet** statt ignoriert (`bg-amber-900/20` über `bg-gray-800` ergibt `rgb(49,43,47)` — aus dem Klassennamen nicht ablesbar); `grep -c "className.includes"` im Messskript liefert **0**. **Die Großtextprüfung ist der Kern des Befundes:** Fund 1 misst **18 px bei Gewicht 700**, liegt damit unter der Schwelle 18,66 px und fällt unter **4,5**, nicht 3,0 — genau daran ist der Abnahmelauf hängengeblieben; beide Zahlen stehen als Zahlen im Nachweis. Dunkelmodus **hergestellt, nicht angenommen** (Umschalter geklickt, `classList.contains('dark') === true` protokolliert, bei `false` Abbruch). **Gegenmessung über alle fünf Flächen in sieben Zuständen und beiden Modi: 3.942 Knoten, davon 1.566 Textknoten im Gate — null unter ihrer Grenze.** Kleinster Wert **hell 4,51**, **dunkel 4,83**. **Das Verfahren ist gegen den Ausgangslauf validiert:** der Lauf trifft beide Enden der Ausgangsbandbreite **exakt** (4,51 und 17,74). **Die Gegenmessung hat drei weitere Unterschreitungen derselben Art gefunden**, die die Sechserliste nicht kannte und die im selben Commit stecken: `text-gray-500` im Vorschaupanel auf `blue-50`/`red-50` (4,44/4,42 → 6,94/6,91 — der Abnahmelauf hat das Panel nur im Zustand „rückwirkend" gemessen, wo derselbe Ton auf `amber-50` 4,66 trägt) und — der bemerkenswerte Fall — `documentedDeltaToneClass()` in `overtimeTransactionFormat.ts:61` mit **3,30**, zeichengleich dem Ausgangswert von Fund 2: die **vierte** Fund-2-Stelle, die nicht in der Komponentendatei steht, sondern im Hilfsmodul daneben. Ohne Gegenmessung wäre D-1 mit einer Unterschreitung von 3,30 im Kontoauszug als geschlossen gemeldet worden. **Scope Fence gehalten:** `sizeStyles` in `Button.tsx` ist zeichengleich (D-2 bleibt Plan 14.2-11); `tailwind.config.js` unangetastet. Drei Commits (`48a8d58` fix über 9 Dateien und einzeln revertierbar, `de625de` Messskript + `14.2-NACHWEIS-D1.md`, der Abschlussgate-Commit mit deferred-items (dieser Eintrag selbst — sein Hash kann sich nicht selbst nennen)), alle vier Gates grün (**578 grün / 3 rot** — genau die vorbestehenden, rote Menge nicht gewachsen), Messskript **Exit 0** mit 22/0 Prüfungen. **D-01: alle fünf Zeilenzahlen und SHA-256 vorher = nachher, ohne jede Wiederherstellung** — das Messskript speichert nichts (Vorschauen sind Trockenläufe, jeder Dialog wird abgebrochen, der Fehlerzustand über `page.route` erzeugt). **Nicht erfüllt und benannt:** E2E 12 grün / 9 rot / 2 übersprungen — byte-genau der Ausgangsstand der Phase, dieselben neun Fälle; `Button.tsx` ist in nahezu jeder Maske im Einsatz, kein neuer roter Fall. **Der Designvertrag `13-UI-SPEC.md` ist an zwei Stellen überholt** (Zeile 131 Accent dunkel, Zeile 166 Gutschrift hell) — benannt, **nicht** hier geändert, wird in Plan 14.2-12 (V-1) nachgezogen. Vier zurückgestellte Nebenbefunde: leere Tailwind-Palette, `text-green-600` mit denselben 3,30 in rund 35 Dateien außerhalb der fünf Flächen, `text-gray-400`-Symbole unter WCAG 1.4.11, `text-gray-500` in `AbsencesBreakdown`. Fünf Dominanzurteile für die UAT-Sammlung (P12-21, 13-U1b, 13-U9 und zwei neue, die erst durch die Korrektur entstehen). Kein Push.
- **Davor abgeschlossen:** Plan 14.2-09 — **F-8 geschlossen.** Die Kollisionsinformation erreicht den Anwender jetzt an zwei Stellen statt an keiner. **(a)** Ein neues Kollisionspanel im Wechsel-Dialog nennt die belegende Periode in Zahlen (`Gültig ab 17.8.2026 bis 11.11.2026 · 30 h/Woche`), trägt `role="status"` (nicht `alert` — der Feldfehler ist bereits die Warnung) und hat für jede Farbklasse eine `dark:`-Entsprechung. **(b)** `resetForm()` nimmt die Zeilenmarkierung nicht mehr unbedingt zurück; die Rücknahme wandert an den kollisionsfreien Vorschauerfolg, den Erfolgspfad von `performSave` und — als Endstelle — an `EditUserModal.handleClose` (`:783`, am Quelltext gefunden statt geraten: X-Knopf, Backdrop, ESC laufen alle über diesen einen Handler). **Die z-Index-Stapelung wurde NICHT gedreht** — `z-[60]` über `z-50` ist unter P12-31/P12-5 abgenommen. **Nachweis ausschließlich mit `document.elementFromPoint()`** (D-10), vier Zustände, 27 Messungen, Exitcode 0: Zustand 1 trifft das Panel (`P.text-sm.text-red-900.dark:text-red-100`) statt eines darüberliegenden Elements; Zustand 2 nach dem Schließen `bg rgb(254,242,242)` / `borderLeft 4px rgb(239,68,68)` gegen den Ausgangsbefund `rgba(0,0,0,0)` / `0px`; Zustand 3 (freier Stichtag) keine Zeile mehr markiert; Zustand 4 im Dunkelmodus, nachdem `classList.contains('dark') === true` **protokolliert** war. **Der neue Textknoten selbst gemessen: 9,16 hell / 11,72 dunkel** (Soll 4,5) — die D-1-Bilanz von Plan 10 wird nicht verschlechtert. Feldfehler und Sperre des Speichern-Knopfes in beiden Modi mitgemessen und unverändert (D-10). Vier Commits (`1426e24` Panel + resetForm, `8c7e453` conflictPeriodId überlebt das Schließen, `960c544` Messskript + `14.2-NACHWEIS-F8.md`, `d50b454` Abschlussgate + deferred-items), alle vier Gates grün (**578 grün / 3 rot** — genau die vorbestehenden, rote Menge nicht gewachsen). **D-01: alle fünf Zeilenzahlen und SHA-256 vorher = nachher, ohne jede Wiederherstellung** — die Messung speichert nichts (die Kollision entsteht schon beim Eintragen des Stichtags), der in Welle 8 beobachtete Urlaubskonten-Nebeneffekt trat deshalb nicht auf. **Nicht erfüllt und benannt:** E2E 12 grün / 9 rot / 2 übersprungen — byte-genau der Ausgangsstand der Phase, keine Spec-Zusicherung betrifft die geänderten Dialoge. `WorkTimePeriodEditModal.tsx` trägt dasselbe Muster (`:391`, `:281`) — geprüft, mit Zeilennummern vermerkt, **nicht** mitrepariert (D-02). Kein Push.
...
- [Phase 14.2-02]: reactivateUser() erweitert statt eines zweiten Endpunkts - Bedingung (deletedAt IS NOT NULL OR status = 'inactive'), aktiver Nutzer bleibt bewusst 404
- [Phase 14.2-02]: updateUserStatus (deletedAt IS NULL-Guard) unangetastet gelassen - Trennung der beiden Wege ist die Sicherheitseigenschaft aus dem Threat Model
...
- [Phase 14.2-07]: F-7 Weg A umgesetzt: sourceStatus-Feld (clientseitig, nicht am Server) waehlt die Erfolgsmeldung; Datenbank speichert weiterhin status='rejected' (D-09, UAT-Punkt Weg B)
- [Phase 14.2-07]: Vier Testantraege (ids 12397-12400) bleiben in absence_requests stehen, weil AbsencesPage.tsx fuer status='rejected' keine Loesch-Schaltflaeche zeigt - nicht per DELETE geglaettet, als UAT-Punkt dokumentiert
- [Phase 14.2-08]: F-2: Periodenaufloesung fuer Listen in EINER SQL-Abfrage (korrelierte Unterabfragen, gebundenes Berlin-Datum, ORDER BY validFrom ASC) statt eines Hooks je Listenzeile — kein N+1, keine Hook-Schleife
- [Phase 14.2-08]: F-2: drei additive optionale Felder currentWeeklyHours/currentWorkSchedule/currentValidFrom als Anzeigekontext neben den unveraenderten Stammdatenfeldern
- [Phase 14.2]: 14.2-09 (F-8): Die z-Index-Stapelung z-[60] ueber z-50 wurde NICHT gedreht (P12-31/P12-5 abgenommen) — die Kollisionsinformation erreicht den Anwender stattdessen als eigenes Panel IM Wechsel-Dialog.
- [Phase 14.2]: 14.2-09 (F-8): Die Ruecknahme der Kollisionsmarkierung wandert aus resetForm() an drei fachlich richtige Stellen (kollisionsfreier Vorschauerfolg, Erfolgspfad von performSave, EditUserModal.handleClose als Endstelle) — sie verschwindet nicht.
...
- [Phase ?]: 14.2-13: Der Produktweg DELETE /api/users/:id ist ein Soft Delete und loest eine UNIQUE-Kollision nicht; die 19 Altlast-Testnutzer wurden ueber PUT /api/users/:id umbenannt statt geloescht — ein physisches Loeschen haette ueber ON DELETE CASCADE 34 vacation_balance- und 32 vacation_transactions-Zeilen mitgenommen und D-01 zerstoert.
- [Phase ?]: 14.2-13: Das ROADMAP-Kriterium 'Die drei E2E-Dateien laufen ohne roten Fall' ist als EINGESCHRAENKT ERFUELLT ausgewiesen — 21 gruen / 0 rot / 2 uebersprungen (Exit 0), aber nur nach dem Freiraeumen von 19 Altlastnamen; ohne den Schritt liefert derselbe Lauf 12/9/2. Zwei Faelle tragen weiterhin test.skip.

## Quick Tasks Completed

| Datum | Slug | Ergebnis |
|-------|------|----------|
| 2026-08-31 | restbefunde-09-1-review | Sieben Restbefunde aus `09.1-REVIEW.md` behoben — sieben unvermischte Commits plus SUMMARY (`ffe29e4`, `80da0b7`, `54b970b`, `04b441e`, `8a06d38`, `efb2e86`, `c5a12d9`, `f4c5ef5`), **nicht gepusht**. Der Push wurde bewusst zurückgestellt: er würde `deploy-server.yml` auslösen, PM2 neu starten und die PID wechseln — und damit die laufende D-17-Beobachtung (Plan 09.1-08, Befund D) zum dritten Mal zurücksetzen. **WR-05** (der gewichtigste): `process.on('SIGTERM'/'SIGINT')` steht jetzt am Anfang von `startServer()` statt am Ende — vorher liefen `seedDatabase()`, `runMigrations()` und `initializeHolidays()` (Netzabruf) ohne Signalbehandler, ein SIGTERM in diesem Fenster tötete den Prozess ohne `wal_checkpoint(TRUNCATE)`. Zur Laufzeit nachgewiesen: `🛡️ Signalbehandlung aktiv` erscheint jetzt vor `🌱 Seeding database...`. Der Fehlerpfad ruft `shutdownDatabase()` vor `process.exit(1)`. **CR-02:** `db.close()` aus `fixOvertime.ts` entfernt — beim harten `process.exit()` läuft `sqlite3_close` nicht, die Dateizeiger eines gleichzeitig laufenden Servers bleiben damit intakt. **WR-14:** `cut -d= -f2` → `-f2-` (kürzte jedes Secret am ersten `=`), und ein neu erzeugtes `SESSION_SECRET` wird jetzt nach `.env` zurückgeschrieben — vorher flogen bei jedem Deployment alle Sitzungen raus, ohne Spur im Log. **CR-04:** `sync-db.ts` liest `SYNC_SOURCE_DB` statt des unmigrierten Altbestands `server/database.db`, Sicherungsname mit Uhrzeit plus Existenz-Guard. **WR-02:** `db:sync:remote`/`syncRemote()` ersatzlos entfernt zugunsten von `scripts/sync-dev-db.sh`. **WR-07:** Fehlerobjekt wird ausgegeben, Close-Verhalten in `migrate.ts`/`seed.ts` vereinheitlicht. **WR-03:** verbotenes `toISOString().split('T')[0]` aus `migrate.ts:createMigration()` entfernt. Der scheinbare Widerspruch CR-02 (Close entfernen) vs. WR-07 (Close ergänzen) ist kontextabhängig aufgelöst und in beiden Dateien als Kommentar hinterlegt: `fixOvertime.ts` läuft neben einem aktiven Server, `migrate.ts`/`seed.ts` seit CR-03 hinter `pm2 stop`. **Verifiziert:** `tsc --noEmit` sauber, Testsuite 592 grün / 3 rot — echte Teilmenge der vier Baseline-Fehlschläge aus der geteilten `development.db`, keine neuen. Kein `ssh`, kein `production.db`-Zugriff, kein Push. Offen bleiben CR-01-nahe Punkte nicht mehr; unbearbeitet: WR-01, WR-04, WR-06, WR-08 bis WR-13, WR-15 und die acht INFO-Punkte. Siehe `.planning/quick/260831-t5j-restbefunde-09-1-review/`. **Ausgeliefert am 02.09.2026 (Lauf `33687538987`, `success`, Serverstand `4d8b3de`).** Zwei Fehlversuche gingen voraus (`33687054408`, `33687346784`), beide brachen beim `git fetch origin main` auf dem Server ab (`could not read Username`, Status 128) — **vor** `pm2 stop`, vor dem Build und vor jedem Datenbankzugriff; die Produktion blieb dabei unberührt. Ursache eingegrenzt (rein lesend, mit `--dry-run`): `git ls-remote` scheitert, `git -c protocol.version=0 ls-remote` gelingt, `git -c protocol.version=0 fetch --dry-run` scheitert weiterhin, `git -c http.version=HTTP/1.1 fetch --dry-run` gelingt. Das GET auf `info/refs` geht also durch, erst der POST auf `git-upload-pack` bricht — **HTTP/2 zwischen git 2.34.1 (Ubuntu 22.04) und GitHub**, nicht die Protokollversion. Der erste Behebungsversuch `ed5a522` (protocol.version=0) traf die falsche Ursache und änderte nur das Fehlerbild; `4d8b3de` erzwingt stattdessen HTTP/1.1 in beiden Deploy-Workflows und lässt die Protokollversion auf dem schnelleren Standard v2. Lauf 33371111657 vom 31.08. lief noch durch, die Änderung liegt damit auf GitHub-Seite. **Nach der Auslieferung geprüft:** PID 25527, Dateizeiger ohne `(deleted)`, Health HTTP 200 in 2,75 ms, crontab unverändert eine Zeile; WR-05 im Protokoll belegt (`🛡️ Signalbehandlung aktiv` **vor** `🌱 Seeding database...`), CR-02 belegt (`grep -c 'db.close()'` in `dist/scripts/fixOvertime.js` = 0), Anlauf `serverstart` 15/15 in 3.334 ms. Vorab-Sicherung dreifach und geprüft: App-Backup `database-backup-2026-09-02T21-33-37-052Z.db` (md5 `5a09600…`, `integrity_check: ok`, 22 Tabellen), Zweitkopie `/home/ubuntu/databases/backups/pre-t5j-deploy_20260902_213337.db`, plus lokale Kopie beim Anwender — der Download war byte-identisch, was zugleich den v1.9.1-Binärdownload an einer echten Binärdatei bestätigt. |
| 2026-08-30 | cr-01-und-cr-03-aus-09-1-review-md-schli | CR-01, CR-03 und WR-16 aus `09.1-REVIEW.md` geschlossen — drei unvermischte Commits (`ff02de5`, `66f0867`, `aa46319`), **nicht gepusht**. **CR-01:** Der crontab-Bereinigungsblock ist aus beiden Deploy-Workflows ersatzlos gestrichen (Entscheidung des Anwenders, nicht die im Review vorgeschlagene Härtung) — `crontab`-Aufrufe je Datei 1 → **0**, an seiner Stelle steht ein Kommentar mit der Begründung und den beiden Belegstellen. Die Prämisse ist belegt, nicht angenommen: `09.1-NACHWEIS-PRODUKTION.md:215` und `09.1-NACHWEIS-STAGING.md:165` zeigen beide `grep -c fix-overtime` = 0. **WR-16:** `scripts/database/setup-cron.sh` entfernt (38 Zeilen), `scripts/README.md` und `ENV.md` auf den In-Prozess-Scheduler umgeschrieben — 0 Verweise auf `setup-cron`/`fix-overtime` in beiden Dateien. **CR-03:** `migrate:prod` und `validate:schema` stehen jetzt hinter `pm2 stop`/`pm2 delete`; gemessene Reihenfolge server 169 < 180 < 190 < 195, staging 163 < 174 < 184 < 189 (vorher stand `migrate` 52 bzw. 51 Zeilen **vor** `pm2 stop`). Der Startaufruf ist zur Shell-Funktion `start_server()` zusammengezogen und wird aus Erfolgs- **und** Fehlerpfad gerufen: schlägt die Migration bei gestopptem Server fehl, wird der Server wieder gestartet und der Lauf endet trotzdem mit `exit 1` — ein fehlgeschlagenes Deployment erzeugt so keinen Ausfall. `pm2 start` steht je Datei genau **einmal** im Quelltext. `migrate.ts` und `validateSchema.ts` tragen den Kopfkommentar „nur bei gestopptem Server", bei `validateSchema.ts` ausdrücklich einschließlich `readonly`. **Verifiziert ohne Ausführung:** YAML parst, `bash -n` grün über 188 bzw. 163 Skriptzeilen, `tsc --noEmit` sauber, `.ts`-Diff ausschließlich Kommentarzeilen. **Die Workflows wurden nicht gefahren** — die Änderungen werden beim nächsten regulären Deployment zum ersten Mal scharf. Restrisiken benannt in `260830-cg6-SUMMARY.md`: verlängertes Wartungsfenster, Start auf einem Stand mit fehlgeschlagener Migration, Staging-Backup auf dem Altbestand (IN-07, nicht im Umfang). **Ausgeliefert und in Produktion belegt (31.08.2026, Lauf 33371111657 gruen, 5m24s):** Alle Schritte `success`, auch „Verify DB path post-deploy" (`DB verification PASSED`), der bei den vorherigen Laeufen nie erreicht wurde. **CR-03 am laufenden Produktionsprozess nachgewiesen:** die Dateizeiger von PID 4911 verweisen auf `production.db`, `-wal` und `-shm` — **kein `(deleted)`**, die WAL des Servers ist unversehrt; die Migration lief bei gestopptem Server (`No pending migrations`). Health 3000: HTTP 200 in 3 ms, beide pm2-Dienste online mit 0 Neustarts, Swap-Nutzung 67 MB. Vorgeschichte:  zwei Staging-Deploys (`33309624652`, `33322533657`) brachen im `tsc`-Build im command_timeout ab und erreichten `pm2 stop` nie — der Umbau ist damit **unerprobt**. Ein dritter Anlauf war noetig, weil Lauf 33352481708 rot endete, obwohl er erfolgreich war — der Health-Check pruefte 03:06:49, der Server meldete `Listening` um 03:06:50, eine Sekunde zu spaet; `130d252` ersetzt den Einzelversuch durch eine Warteschleife und gibt curl ein `|| echo 000`, ohne das `script_stop` vor der Fehlerauswertung abbrach (derselbe Mechanismus liess schon Lauf 33270724872 am 29.08. scheitern). Zusatzcommit `ad8adc1` setzt `command_timeout: 20m` in beiden Workflows. Unabhängig davon entfernt `a9c85fd` den toten Schema-Validierungsschritt ganz (parallel entstandener Vorgang, hier nur sauber geschnitten) — damit entfällt der zweite der beiden Fremdprozesse aus CR-03 vollständig, statt nur besser terminiert zu sein; die Verlagerung von `migrate:prod` bleibt nötig und unberührt. Danach war der Host nicht mehr ansprechbar, siehe „Ungeplante Eingriffe (Produktion)", Eintrag 2026-08-30. `origin/staging` steht auf `1293c41` (vor dem Neuschnitt der Historie), `origin/main` unverändert auf `e4866a4`. Siehe `.planning/quick/260830-cg6-cr-01-und-cr-03-aus-09-1-review-md-schli/`. |
...
- **2026-08-30 Server nicht mehr ansprechbar nach zwei Staging-Deploys (behoben):** Im Rahmen von
  ...
  verwenden dasselbe `secrets.ORACLE_HOST`. Produktion (`TimeTracking-Clean`, Port 3000) und
  Staging (`TimeTracking-Green`, Port 3001) liegen auf **einer** Maschine (129.159.8.19); beide
  Builds rechneten damit auf dem Produktionshost. Beide Laeufe brachen im `tsc`-Build im
  `command_timeout` ab (Lauf 1 nach 10 min Default, Lauf 2 nach 20 min trotz erhoehtem Limit —
  `tsc` lief dort 15m51s ohne fertig zu werden, `npm install` meldete "up to date" nach 4 min).
  **`pm2 stop` wurde in keinem der beiden Laeufe erreicht**, der Umbau aus CR-01/CR-03 ist also
  weiterhin unerprobt und hat den Zustand nicht verursacht. Danach war die Maschine ueber eine
  Stunde nicht ansprechbar: Port 22 TCP offen, aber `Connection timed out during banner exchange`
  auch bei 120 s Geduld; Ports 3000 und 3001 ohne Antwort. Vermutete Ursache (nicht belegt, weil
  kein Zugang): verwaiste `tsc`-Prozesse — der SSH-Timeout beendet die Sitzung, nicht zwingend
  den Prozess auf dem Server. **Kein Datenbankzugriff erfolgt, kein Deployment auf `main`
...
| Phase 14.2-restbefunde-der-abnahme-schliessen P07 | 40min | 3 tasks | 4 files |
...
| Phase 14.2 P13 | 34 | 3 tasks | 5 files |
