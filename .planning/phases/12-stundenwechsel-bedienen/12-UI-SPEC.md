...
| `desktop/src/pages/UserManagementPage.tsx` | Der lokale State hält nur noch die **ID**: `const [editingUserId, setEditingUserId] = useState<number \| null>(null)` (heute Zeile 38: `useState<User \| null>`). Das Objekt wird bei jedem Render aus der bestehenden Nutzerquery abgeleitet: `const editingUser = users?.find(u => u.id === editingUserId) ?? null;`. Zeile 464 `setEditingUser(user)` → `setEditingUserId(user.id)`. Zeile 523–527 bleiben in ihrer Form (`{editingUser && <EditUserModal … user={editingUser} />}`). | Rein lokale Zustandsführung derselben Seite, keine Prop- oder Signaturänderung nach außen. `EditUserModal` bekommt weiterhin ein vollständiges `User`-Objekt. Verschwindet der Nutzer aus der Liste (Löschung), liefert `find` `undefined`, das Modal schließt und `editingUserId` wird zurückgesetzt — das ist in genau diesem Fall auch das richtige Verhalten. |
...
Mangel gewertet werden): `EditUserModal.tsx` enthält in Zeile 103–106 und 123 `console.log`-
Debugausgaben und nutzt an mehreren Stellen starre `grid-cols-2`/`grid-cols-3` ohne Breakpoint;
`OvertimeTransactions.tsx` typisiert Transaktionen als `any`. Diese Stellen liegen im
Absende-/Renderpfad bestehender Funktionen und werden in dieser Phase bewusst nicht angefasst
(„NO REGRESSION"). Neuer Code dieser Phase führt keines dieser Muster fort: keine neuen
...
> gegenstandslos, nicht widersprüchlich.
